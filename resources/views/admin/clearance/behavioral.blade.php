...
@section('title', 'Behavioral Clearance')
...
<div class="flex-1 overflow-y-auto lg:p-8 dark:bg-dark-bg bg-slate-50/50 p-4">
    ...
    <x-admin.page-header title="Clearance" subtitle="Student Requirements Validation">
        <div class="flex items-center gap-2 mt-1 sm:mt-0" x-data="{ open: false }">
            ...
            <form method="GET" action="{{ route('admin.clearance.behavioral') }}" id="sy-form">
    ...
    <div class="rounded-2xl border border-slate-200 dark:border-dark-border bg-white dark:bg-dark-card shadow-sm overflow-hidden">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 px-6 py-4 border-b border-slate-100 dark:border-dark-border">
            <div class="flex items-center gap-3">
                <div class="flex h-9 w-9 items-center justify-center rounded-xl bg-purple-50 dark:bg-purple-900/20">
                    <iconify-icon icon="solar:shield-user-bold" width="18" class="text-purple-600 dark:text-purple-400"></iconify-icon>
                </div>
                <span class="text-base font-semibold text-slate-800 dark:text-white">Behavioral</span>
            </div>
        ...
        {{-- ── Filters ── --}}
        <form method="GET" action="{{ route('admin.clearance.behavioral') }}" id="filter-form"
            ...
            {{-- Incident Severity --}}
            <div class="flex flex-col gap-1">
                <label class="text-[11px] font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide">Incident Severity</label>
                <div class="relative">
            ...
            <div class="flex flex-col gap-1">
                ...
                <div class="flex gap-2">
                    ...
                    <a href="{{ route('admin.clearance.behavioral', ['school_year' => $schoolYear]) }}"
            ...
            <div class="flex items-center gap-2 flex-wrap">
                <div class="flex rounded-lg overflow-hidden border border-slate-200 dark:border-dark-border text-xs font-semibold">
                    @foreach($levels as $val => $label)
                        <a href="{{ route('admin.clearance.behavioral', array_merge(request()->except('program_level','page'), ['school_year'=>$schoolYear, 'program_level'=>$val])) }}"
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left text-sm" style="min-width: 1050px">
                <thead>
                    <tr class="border-b border-slate-100 dark:border-dark-border text-[11px] font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400 bg-slate-50/70 dark:bg-white/[0.02]">
                        ...
                        <th class="px-4 py-3 whitespace-nowrap text-center">Total Incidents</th>
                        <th class="px-4 py-3 whitespace-nowrap">Last Incident</th>
                        <th class="px-4 py-3 whitespace-nowrap">Behavior Type</th>
                        <th class="px-4 py-3 whitespace-nowrap text-center">Severity</th>
                        <th class="px-4 py-3 whitespace-nowrap text-center">Incident Status</th>
                        <th class="px-4 py-3 whitespace-nowrap text-center">Parent Notified</th>
                        <th class="px-4 py-3 whitespace-nowrap">Recorded By</th>
                        <th class="px-4 py-3 whitespace-nowrap">Cleared By</th>
                        <th class="px-4 py-3 whitespace-nowrap text-center">Clearance</th>
                ...
                @php
                    $records   = $student->behavioralRecords;
                ...
                <tr class="hover:bg-slate-50/70 dark:hover:bg-white/[0.02] transition-colors group">
                    ...
                    <td class="px-4 py-3 text-center">
                        @if($incidents > 0)
                            <span class="inline-flex items-center justify-center h-6 w-6 rounded-full bg-red-100 dark:bg-red-900/30 text-xs font-bold text-red-700 dark:text-red-400">
                                {{ $incidents }}
                            </span>
                        @else
                    ...
                    <td class="px-4 py-3 text-xs text-slate-500 dark:text-slate-400 whitespace-nowrap">
                        @if($latest)
                            <div>{{ $latest->incident_date?->format('M d, Y') ?? '—' }}</div>
                            <div class="text-[10px] text-slate-400">{{ $latest->incident_date?->diffForHumans() }}</div>
                        @else
                            <span class="text-slate-300 dark:text-slate-600">No incidents</span>
                        @endif
                    </td>
                    <td class="px-4 py-3 text-xs text-slate-600 dark:text-slate-300">
                        @if($latest)
                            <div class="max-w-[140px] truncate" title="{{ $latest->behavior_type }}">
                                {{ $latest->behavior_type ?? '—' }}
                            </div>
                            @if($incidents > 1)
                                <div class="text-[10px] text-slate-400">+{{ $incidents - 1 }} more</div>
                            @endif
                        @else
                            <span class="text-slate-300 dark:text-slate-600">—</span>
                        @endif
                    ...
                    <td class="px-4 py-3 text-center">
                        @if($latest?->severity)
                            <span class="inline-flex items-center rounded-full px-2 py-0.5 text-[11px] font-semibold
                                {{ $severityBadge[$latest->severity] ?? 'bg-slate-100 text-slate-600' }}">
                                {{ $latest->severity }}
                            </span>
                        @else
                    ...
                    <td class="px-4 py-3 text-center">
                        @if($latest?->status)
                            <span class="inline-flex items-center rounded-full px-2 py-0.5 text-[11px] font-semibold
                                {{ $incidentStatusBadge[$latest->status] ?? 'bg-slate-100 text-slate-600' }}">
                    ...
                    <td class="px-4 py-3 text-xs text-slate-600 dark:text-slate-300 whitespace-nowrap">
                        {{ $student->behavioral_cleared_by ?? '—' }}
                    </td>
                    <td class="px-4 py-3 text-center">
                        @php $status = $student->behavioral_clearance ?? 'pending'; @endphp
                        <span class="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-semibold
                            {{ $clearanceBadge[$status] ?? $clearanceBadge['pending'] }}">
                            {{ ucfirst($status) }}
                        </span>
                    </td>
                    <td class="px-4 py-3 text-center">
                        <div x-data="{ open: false }" class="relative inline-block">
                            ...
                            <div x-show="open"
                                ...
                                style="display:none">
                                <button onclick="updateBehavioralClearance({{ $student->id }}, 'cleared')"
                                ...
                                <button onclick="updateBehavioralClearance({{ $student->id }}, 'pending')"
                                    class="flex w-full items-center gap-2 px-3 py-2 text-xs text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-white/5 transition-colors">
                                    <iconify-icon icon="solar:clock-circle-bold" width="14" class="text-amber-500"></iconify-icon>
                                    Mark Pending
                                </button>
                                <button onclick="updateBehavioralClearance({{ $student->id }}, 'overdue')"
                                    class="flex w-full items-center gap-2 px-3 py-2 text-xs text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-white/5 transition-colors">
                                ...
                                <a href="{{ route('admin.student-records.behavioral') }}?search={{ urlencode($student->full_name) }}"
                                    class="flex w-full items-center gap-2 px-3 py-2 text-xs text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-white/5 transition-colors">
                                    <iconify-icon icon="solar:document-text-bold" width="14" class="text-purple-500"></iconify-icon>
                                    View Records
                                </a>
                            </div>
                        </div>
                    ...
                    <td colspan="12" class="px-6 py-16 text-center">
                        <div class="flex flex-col items-center gap-3">
                            <div class="flex h-14 w-14 items-center justify-center rounded-2xl bg-slate-100 dark:bg-slate-800">
                                <iconify-icon icon="solar:shield-user-bold" width="28" class="text-slate-400"></iconify-icon>
                            </div>
...
<script>
    ...
    function updateBehavioralClearance(id, status) {
        fetch(`/admin/clearance/behavioral/${id}`, {
            method: 'PATCH',
        ...
        .then(d => {
            if (d.success) {
                ...
                toast.className = 'fixed top-6 right-6 z-[200] rounded-xl border border-green-200 bg-green-50 px-4 py-3 text-sm text-green-700 shadow-xl';
