    ...
    <div class="rounded-2xl border border-slate-200 dark:border-dark-border bg-white dark:bg-dark-card shadow-sm overflow-hidden">
        ...
        <div class="px-6 pt-6 pb-4 border-b border-slate-100 dark:border-dark-border">
            <div class="flex items-center gap-2 mb-1">
                <iconify-icon icon="solar:upload-minimalistic-bold" width="18" class="text-slate-600 dark:text-slate-300"></iconify-icon>
                <h2 class="text-base font-bold text-slate-800 dark:text-white">Promote Student</h2>
            </div>
            <p class="text-xs text-slate-400 dark:text-slate-500 ml-6">Promote Student to Next Grade Level/School Year</p>
        </div>
        ...
        <div class="px-6 py-5 border-b border-slate-100 dark:border-dark-border">
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4 items-end">
                ...
                <div class="flex flex-col gap-1.5">
                    ...
                    <div class="rounded-lg border border-slate-200 dark:border-dark-border bg-slate-50 dark:bg-dark-card px-3 py-2 text-sm text-slate-500 dark:text-slate-400 cursor-not-allowed">
                        2025 - 2026
                    </div>
                </div>
                ...
                <div class="flex flex-col gap-1.5">
                    ...
                    <div class="relative">
                        <select class="w-full appearance-none rounded-lg border border-slate-200 dark:border-dark-border dark:bg-dark-card dark:text-slate-300 px-3 py-2 text-sm text-slate-600 focus:outline-none focus:ring-2 focus:ring-blue-500 pr-8">
                            <option value=""></option>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left text-sm" style="table-layout:fixed;min-width:700px">
                <colgroup>
                ...
                <thead>
                    <tr class="bg-slate-50 dark:bg-white/5 border-b border-slate-100 dark:border-dark-border text-xs font-semibold text-slate-500 dark:text-slate-400">
                        ...
                        <th class="px-4 py-3">Final Results</th>
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-dark-border">
                    ...
                    @php
                        $students = [
                            ['232-555-09', 'Jenny Orquiola',  'STEM (12)', 'Passed', 'Pending'],
                            ['232-555-09', 'Jeneva Ybanez',   'Grade 6',   'Passed', 'Pending'],
                            ['232-555-09', 'Dianne Balaoro',  'Grade 9',   'Passed', 'Cleared'],
                            ['232-555-08', 'Hans Gayon',      'Kinder',    'Passed', 'Pending'],
                        ];
                    ...
                    <tr class="hover:bg-slate-50 dark:hover:bg-white/5 transition-colors">
                        ...
                        <td class="px-4 py-3">
                            <span class="inline-flex items-center rounded-full bg-slate-100 dark:bg-slate-700 px-2.5 py-0.5 text-xs font-medium text-slate-600 dark:text-slate-300">
                                {{ $row[3] }}
                            </span>
                        </td>
                        <td class="px-4 py-3">
                            <span class="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-semibold
                                {{ $clearanceClass[$row[4]] ?? 'bg-slate-100 text-slate-600' }}">
                                {{ $row[4] }}
                            </span>
                        </td>
                        <td class="px-4 py-3">
                            <div class="flex items-center justify-center gap-2">
                                ...
                                <button title="Promote"
                                    class="promote-btn flex items-center gap-1.5 rounded-lg
        ...
        {{-- ── Bulk Promote Bar — shown when checkboxes checked ── --}}
        <div id="bulk-promote-bar"
             class="hidden items-center justify-between px-6 py-4
            ...
            <div class="flex items-center gap-2">
                ...
                <span id="selected-count" class="text-sm font-medium text-green-700 dark:text-green-300">0 student(s) selected</span>
            </div>
            <button class="flex items-center gap-2 rounded-xl bg-green-600 hover:bg-green-700
                           px-5 py-2 text-sm font-semibold text-white transition-colors shadow-sm">
                <iconify-icon icon="solar:upload-minimalistic-bold" width="16"></iconify-icon>
                Promote Selected
            </button>
        </div>
...
<script>
    ...
    const bulkBar       = document.getElementById('bulk-promote-bar');
    ...
    function updateBulkBar() {
        ...
        if (count > 0) {
            ...
            countLabel.textContent = count + ' student' + (count > 1 ? 's' : '') + ' selected';
        } else {
