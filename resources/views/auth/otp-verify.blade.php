...
<head>
  ...
  <title>Verify OTP – Messiah School</title>
  ...
  <style>
    ...
    .otp-box:focus {
      border-color: #0d4c8f;
      box-shadow: 0 0 0 3px rgba(13,76,143,0.12);
    }
...
<body class="font-poppins min-h-screen overflow-hidden bg-glass-gradient flex flex-col">
  ...
  <main class="flex-1 flex items-center justify-center px-4" style="margin-top:-60px">
    <div class="relative w-[410px] rounded-none bg-white shadow-card border border-white/20 backdrop-blur-xl overflow-hidden animate-fadeUp">
      ...
      {{-- Warning --}}
      @if(session('warning'))
        <div id="alert-warning" class="mx-6 mt-5 flex items-center gap-3 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3">
          ...
          <p class="flex-1 text-xs text-amber-700">{{ session('warning') }}</p>
        </div>
      @endif
      ...
      <div class="relative z-8 flex flex-col px-6 pb-10 pt-5">
        ...
        <h1 class="text-3xl font-bold text-gray-800 mb-1 leading-tight">Verify OTP</h1>
        <p class="text-xs text-gray-500 mb-6 leading-relaxed">
          Enter the 6-digit code sent to<br>
          <span class="font-semibold text-gray-700">{{ $maskedIdentifier }}</span>
          via Email.
          <br>Code expires in <span id="countdown" class="font-bold text-red-500">10:00</span>.
        </p>
        ...
        {{-- OTP Form --}}
        <form method="POST" action="{{ route('password.verify.store') }}" id="otp-form">
          @csrf
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    boxes.forEach((box, i) => {
      box.addEventListener('input', () => {
        const val = box.value.replace(/\D/, '');
        box.value = val;
        if (val) { box.classList.add('filled'); if (boxes[i+1]) boxes[i+1].focus(); }
    ...
    // ── Auto-dismiss alerts ──────────────────────────────────
    ['alert-status','alert-warning','alert-error'].forEach(id => {
